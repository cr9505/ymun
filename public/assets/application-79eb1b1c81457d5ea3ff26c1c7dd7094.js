...
(function() {
  var dangerAlert, hideAlert, successAlert;

  hideAlert = function() {
    return $('.alert-success').delay(6000).slideUp();
  };
  ...
  $(document).ready(function() {
    var eurButton, usdButton;
    $('.chosen-select').chosen();
    $('.add-one').click(function(e) {
      var duplicatableGroup;
      e.preventDefault();
      duplicatableGroup = $('.duplicatable').last().parents('.form-group').clone().show();
      duplicatableGroup.insertBefore($(this).parent());
      duplicatableGroup.find('.duplicatable').each(function() {
        var duplicatable, name;
        duplicatable = $(this);
        name = duplicatable.attr('name').replace(/\[([0-9]+)]/, function(str, index) {
          return "[" + (parseInt(index) + 1) + "]";
        });
        return duplicatable.attr('name', name);
      });
      duplicatableGroup.find('a.delete').click(function() {
        var form_group, index;
    ...
    $('select.with-other').change(function(e) {
      var $this, input;
      $this = $(this);
      input = $("input[name='" + ($this.attr('name')) + "']");
      if ($(this).val() === 'other') {
        return input.attr('disabled', false).show();
      } else {
        return input.attr('disabled', true).hide();
      }
    }).each(function() {
      var $this;
      $this = $(this);
    ...
    if (eurButton = $('#eur-button')) {
      ...
      $('.payment-type-button').click(function() {
        ...
        return $.getJSON('/delegation/change_payment_type.json', {
          payment_type: paymentType
        }, function(data) {
          if (data.success) {
            return successAlert('Payment method was successfully changed!');
          } else {
            return dangerAlert('Something went wrong! Please try again in a little while.');
          }
        });
      });
      if (!eurButton.hasClass('disabled')) {
        usdButton.click(function() {
          $('#payment-usd').show();
          $('#payment-eur').hide();
          usdButton.addClass('active');
          eurButton.removeClass('active');
          $('#pay-with-paypal').show();
          return $.getJSON('/delegation/change_payment_currency.json', {
            currency: 'usd'
          }, function(data) {
            if (data.success) {
              return successAlert('Currency successfully changed to USD!');
            } else {
              return dangerAlert('Something went wrong! Please try again in a little while.');
            }
          });
        });
        eurButton.click(function() {
          $('#payment-eur').show();
          $('#payment-usd').hide();
          ...
          return $.getJSON('/delegation/change_payment_currency.json', {
            ...
            if (data.success) {
              return successAlert('Currency successfully changed to EUR!');
            } else {
              return dangerAlert('Something went wrong! Please try again in a little while.');
            }
          });
    ...
    $('#payment-submit').click(function(e) {
      ...
      if (form.find('input:checked').length) {
        ...
        return dangerAlert('Please select a payment option or amount.');
      }
    });
    ...
    $('.fake-save').click(function() {
      ...
      return setTimeout(function() {
        $this.text(text);
        return successAlert('All information saved!');
      }, 500);
    });
